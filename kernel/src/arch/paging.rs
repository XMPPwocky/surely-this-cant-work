...
pub fn init_kernel_page_table() -> PhysPageNum {
    ...
    pt.map_range(0x8000_0000, 0x8000_0000, text_start - 0x8000_0000, PTE_R | PTE_X).expect("boot: map SBI");
    ...
    pt.map_range(0x1000_0000, 0x1000_0000, PAGE_SIZE, PTE_R | PTE_W).expect("boot: map UART");

    // Identity-map PLIC: 0x0C00_0000..0x1000_0000
    println!("  Mapping PLIC:   {:#x}..{:#x} (R+W)", 0x0C00_0000usize, 0x1000_0000usize);
    pt.map_range(0x0C00_0000, 0x0C00_0000, 0x0400_0000, PTE_R | PTE_W).expect("boot: map PLIC");

    // Identity-map CLINT: 0x0200_0000..0x0201_0000
    println!("  Mapping CLINT:  {:#x}..{:#x} (R+W)", 0x0200_0000usize, 0x0201_0000usize);
    pt.map_range(0x0200_0000, 0x0200_0000, 0x0001_0000, PTE_R | PTE_W).expect("boot: map CLINT");

    // Identity-map VirtIO: 0x1000_1000..0x1000_9000
    println!("  Mapping VirtIO: {:#x}..{:#x} (R+W)", 0x1000_1000usize, 0x1000_9000usize);
    pt.map_range(0x1000_1000, 0x1000_1000, 0x0000_8000, PTE_R | PTE_W).expect("boot: map VirtIO");
